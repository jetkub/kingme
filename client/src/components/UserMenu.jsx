...
const UserMenu = () => {
	...
	// use Auth.getProfile() (returns the decoded JWT payload) to get user data
			...
			>
				<Menu.Items className='absolute right-0 z-10 mt-2 w-72 origin-top-right divide-y-2 divide-neutral-400 rounded-none border-4 border-t-0 border-neutral-800 bg-neutral-200 font-base text-lg shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none'>
					<div className='px-4 py-3'>
						...
						<p className='truncate font-medium text-neutral-800'>{user.email}</p>
					</div>
					<div className='py-1'>
						<Menu.Item>
							...
							{({ active }) => (
								<a
									...
									className={classNames(
										active ? 'bg-neutral-5 text-neutral-900' : 'text-neutral-700',
										'block px-4 py-2'
									)}
								>
